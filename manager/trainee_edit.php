...
// Get id from url
$id = $_GET['id'];
$sql = "SELECT users.id as userId , users.username , users.email , users.password , users.photo , users.phone , users.country_id, users.country_id, users.user_type_id , users.user_created_at ,country.country_name  as country ,trainees_more_info.id as more_id , trainees_more_info.weight ,trainees_more_info.height ,trainees_more_info.package_id , trainees_more_info.coach_id , trainees_more_info.more_info , package.id as packages_id , package.package_name , package.month_num , package.package_amount FROM users  JOIN country ON users.country_id = country.id join trainees_more_info on trainees_more_info.more_info = users.id join package on package.id = trainees_more_info.package_id WHERE users.id = $id";
$result = mysqli_query($conn, $sql);
$data = mysqli_fetch_assoc($result);
$oldImage   = $data['photo'];
$more_id = $data['more_id'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $weight = CleanInputs($_POST['weight']);
    $height = CleanInputs($_POST['height']);
    ...
    } elseif (!Sanitize($weight, 1)) {
        $errors['weight'] = "Weight Field Must be a Number";
    }

    if (!Validator($height, 1)) {
        $errors['height'] = "Height Field Is Required";
    } elseif (!Sanitize($height, 1)) {
        $errors['height'] = "Height Field Must be a Number";
    }

    if (!Validator($package, 1)) {
        $errors['package'] = "Package Field Is Required";
    }
    if (!Validator($coach, 1)) {
    ...
    if (empty($errors)) {
        ...
        // insert data in two Table

        $sql = "update users , trainees_more_info set users.username='$name' , users.email= '$email' , users.password='$password' , users.phone = $phone , users.country_id = $country , users.photo = '$finalName'  , trainees_more_info.weight=$weight , trainees_more_info.height=$height , trainees_more_info.package_id=$package , trainees_more_info.coach_id = $coach  , users.add_by = $related_user where trainees_more_info.more_info =users.id and trainees_more_info.id = $more_id";
        $result = mysqli_query($conn, $sql);
        if ($result) {
                                ...
                                <form class="form" method="POST" action="<?php $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data">
                                    <div class="form-body">
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                <div class="form-label-group">
                                                    <input type="number" id="first-name-column" class="form-control" placeholder="Weight" name="weight" value="<?php echo $data['weight'] ?>">
                                            ...
                                            <div class="col-12">
                                                <div class="form-label-group">
                                                    <input type="number" id="last-name-column" class="form-control" placeholder="Height" name="height" value="<?php echo $data['height'] ?>">
                                            ...
                                            <div class="col-12">
                                                <div class="form-label-group">
                                                    ...
                                                    <select class="select2-theme form-control" id="select2-theme2" name="package">
                                                        <option value="0">Chosee Package</option>
                                                        <?php
                                                        // Select Data for drobdown Menu
                                                        $querys = "SELECT * FROM `package` where add_by = $related_user";
                                                        $results = mysqli_query($conn, $querys);
                                                        while ($row = mysqli_fetch_assoc($results)) {
                                                        ?>
                                                            <option value="<?php echo  $row['id']; ?>" <?php if ($row['id'] == $data['packages_id']) {
                                                                                                            echo 'selected';
                                                                                                        } ?>>
                                            ...
                                            <div class="col-12">
                                                <div class="form-label-group">
                                                    ...
                                                    <select class="select2-theme form-control" id="select2-theme3" name="coach">
                                                        <option value="0">Chosee Coach</option>
                                                        <?php
                                                        // Select Data for drobdown Menu
                                                        $query = "SELECT * FROM `users` where user_type_id = 3 and add_by = $related_user";
                                                        $result = mysqli_query($conn, $query);
                                                        while ($row = mysqli_fetch_assoc($result)) {
                                                        ?>
                                                            <option value="<?php echo  $row['id']; ?>" <?php if ($row['id'] == $data['coach_id']) {
                                                                                                            echo 'selected';
